...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    public UserDetailsService userDetailsService() {
        //获取登录用户信息
        return username -> {
            SysUserVo sysUser = userMapper.selectByUserName(username);
            //  apiContext.setCurrentProviderId(admin.getStoreId());
            if (sysUser != null) {
                if (sysUser.getIsAdmin() != null && sysUser.getIsAdmin() == 1L) {
                    List<SysPermission> permissionList = sysUserService.listPerms();
                    return new AdminUserDetails(sysUser, permissionList);
                }
                List<SysPermission> permissionList = sysUserService.listUserPerms(sysUser.getId());
                return new AdminUserDetails(sysUser, permissionList);
            }
