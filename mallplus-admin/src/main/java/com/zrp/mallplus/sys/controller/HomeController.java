...
import com.zrp.mallplus.vo.OrderStatusCount;
...
public class HomeController extends BaseController {
    ...
    public Object goodsStatic() throws Exception {
        ...
        for (PmsProduct goods : goodsList) {
            ...
            if (new SimpleDateFormat(goods.getCreateTime()).format(new Date()).equals(DateUtils.format(new Date()))) {
                nowCount++;
            }
            if (new SimpleDateFormat(goods.getCreateTime()).format(new Date()).equals(DateUtils.addDay(new Date(), -1))) {
                yesCount++;
