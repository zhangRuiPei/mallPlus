...
import com.zscat.mallplus.bo.PmsProductVo;
...
public class PmsProductCategoryController {
    ...
    public Object getPmsProductCategoryByPage() {
        /*try {
        ...
        List<Map> allMenu = IPmsProductCategoryService.findAllMenu();

        return new CommonResult().success(allMenu);
    }

    @SysLog(MODULE = "pms", REMARK = "根据条件查询所有产品分类列表")
    @ApiOperation("根据条件查询所有产品分类列表")
    @GetMapping(value = "/getList")
    public Object getFenlei() {
        /*try {
            return new CommonResult().success(IPmsProductCategoryService.page(new Page<PmsProductCategory>(pageNum, pageSize), new QueryWrapper<>(entity)));
        } catch (Exception e) {
            log.error("根据条件查询所有产品分类列表：%s", e.getMessage(), e);
        ...
        List<Map> allMenu = IPmsProductCategoryService.findAllFen();
    ...
    @PreAuthorize("hasAuthority('pms:PmsProductCategory:delete')")
    public Object deleteBatch(@RequestBody PmsProductVo pmsProductVo) {
        boolean count = false;
        for (Long longs :pmsProductVo.getId()){
           count = IPmsProductCategoryService.removeByIds(Collections.singleton(longs));
        }
        if (count) {
