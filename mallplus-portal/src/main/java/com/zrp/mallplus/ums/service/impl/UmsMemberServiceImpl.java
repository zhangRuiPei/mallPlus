...
import com.zrp.mallplus.component.UserUtils;
...
import com.zrp.mallplus.ums.mapper.UmsMemberMemberTagRelationMapper;
import com.zrp.mallplus.ums.service.*;
import com.zrp.mallplus.util.*;
import com.zrp.mallplus.utils.CommonResult;
import com.zrp.mallplus.utils.MatrixToImageWriter;
import com.zrp.mallplus.utils.ValidatorUtils;
import com.zrp.mallplus.vo.*;
...
//     private AuthenticationManager authenticationManager;*/
    ...
    public String getWxPhone(String openId, String keyStr, String ivStr, String encDataStr) {
        ...
        try {
            String result = AesCbcUtil.decrypt(encDataStr, keyStr, ivStr, "UTF-8");
            String result1 = AES.wxDecrypt(encDataStr, keyStr, ivStr);
            System.out.println(result);
            System.out.println(result1);
            Gson gson = new Gson();
            wxPhoneInfo = gson.fromJson(result, WxPhoneInfo.class);
            if (wxPhoneInfo != null) {
                System.out.println(wxPhoneInfo.getPhoneNumber());
                UmsMember byOpenId = memberMapper.findByOpenId(openId);
                byOpenId.setPhone(wxPhoneInfo.getPhoneNumber());
                memberMapper.updateById(byOpenId);
    ...
    public CommonResult register(UmsMember user) {
        ...
        try {
            String defaultIcon = "https://yigou-test.oss-cn-beijing.aliyuncs.com/u%3D1377547971%2C3453629665%26fm%3D26%26gp%3D0.jpg";
            umsMember.setIcon(defaultIcon);
            //这是要生成二维码的url
            String url = "https://yigou-test.oss-cn-beijing.aliyuncs.com/u%3D1377547971%2C3453629665%26fm%3D26%26gp%3D0.jpg" + user.getUsername();
            //要添加到二维码下面的文字
    ...
    public Map<String, Object> appLogin(String openid, Integer sex, String headimgurl, String unionid, String nickname, String city, Integer source) {
        ...
        if (null == userVo) {
            ...
            if (StringUtils.isEmpty(headimgurl)) {
                //会员头像(默认头像)
                umsMember.setIcon("https://yigou-test.oss-cn-beijing.aliyuncs.com/u%3D1377547971%2C3453629665%26fm%3D26%26gp%3D0.jpg");
            } else {
    ...
    /**
     * 获取微信OPENID,验证用户信息完整性
     * 对象参数：code,UserInfo,Signature
     * 返回：openid
     */
    @Override
...
//            // 发送请求
//            String wxResult = UrlUtil.sendPost(WX_LOGIN_URL, param);
//            com.alibaba.fastjson.JSONObject jsonObject = com.alibaba.fastjson.JSONObject.parseObject(wxResult);
//            // 获取参数返回的
//            String session_key = jsonObject.get("session_key").toString();
//            String open_id = jsonObject.get("openid").toString();
//
//            WeChatAccessToken accessToken = HttpClientUtil.getAccessToken();
//
//            String wxUserInfoUrlString = HttpClientUtil.getWXUserInfoUrlString(open_id, accessToken.getAccess_token());
            String userInfos = req.getUserInfo();
            ...
            if (null == userVo) {
                ...
                String defaultIcon = "https://timgsa.baidu.com/timg?image&quality=80&size=b9999_10000&sec=1597749043236&di=7785522f6b2d324e9c188d4b22f3ee78&imgtype=0&src=http%3A%2F%2Fa3.att.hudong.com%2F14%2F75%2F01300000164186121366756803686.jpg";
                umsMember.setIcon(defaultIcon);
                //这是要生成二维码的url
                String url = "http://www.yjlive.cn:8082/?XS=" + umsMember.getUsername();
                //要添加到二维码下面的文字
     ...
     */
    @Override
    public Map<String, Object> loginByWeixin2(String code, UserInfo userInfos, String cloudID) {
        try {
            SysAppletSet appletSet = appletSetMapper.selectOne(new QueryWrapper<>());
            if (null == appletSet) {
                throw new ApiMallPlusException("没有设置支付配置");
            ...
            if (null == userVo) {
                ...
                String iconAvatar = "https://yigou-test.oss-cn-beijing.aliyuncs.com/u%3D1377547971%2C3453629665%26fm%3D26%26gp%3D0.jpg";
                if (StringUtils.isEmpty(userInfos.getAvatarUrl())) {
                    //会员头像(默认头像)
                    umsMember.setIcon(iconAvatar);
                } else {
                ...
                umsMember.setIcon(iconAvatar);
                //这是要生成二维码的url
                String url = "http://localhost/?invitecode=" + umsMember.getUsername();
                //要添加到二维码下面的文字
                String words = umsMember.getUsername() + "的二维码";
                //调用刚才的工具类
                ByteArrayResource qrCode = MatrixToImageWriter.createQrCode(url, words);
...
//                     umsMember.setGender(Integer.parseInt(me.get("gender")));
     ...
     * @return
     */
    public Map<String, Object>loginBySms(String phone,String phonecode) {
        Map<String,Object> objectMap = new HashMap<>();
        String token = null;
        UmsMember umsMember1 = memberMapper.findByPhone(phone);
        MemberDetails userDetails = (MemberDetails) userDetailsService.loadUserByUsername(umsMember1.getUsername());
        if (userDetails == null){
            throw new BadCredentialsException("用户名错误");
        }
        if (!verifyAuthCode(phonecode,phone)){
            throw new BadCredentialsException("验证码错误");
        }
        UmsMember member = this.getByUsername(umsMember1.getUsername());
        if (member==null || member.getId()<1){
            throw new ApiMallPlusException("用户不存在");
        }
        addIntegration(member.getId(), logginJifen, 1, "登录添加积分", AllEnum.ChangeSource.login.code(), member.getUsername());
        ...
        SecurityContextHolder.getContext().setAuthentication(authentication);
        token = jwtTokenUtil.generateToken(userDetails);
        objectMap.put("userInfo", member);
        objectMap.put("token", token);
        objectMap.put("tokenHead", tokenHead);
        return objectMap;
    }
