...
import com.zrp.mallplus.exception.ApiMallPlusException;
...
import com.zrp.mallplus.oms.entity.*;
import com.zrp.mallplus.oms.mapper.OmsCartItemMapper;
import com.zrp.mallplus.oms.mapper.OmsOrderMapper;
import com.zrp.mallplus.oms.mapper.OmsOrderReturnApplyMapper;
import com.zrp.mallplus.oms.mapper.OmsOrderSettingMapper;
...
import com.zrp.mallplus.pms.entity.PmsGifts;
...
import com.zrp.mallplus.pms.service.IPmsCommentService;
import com.zrp.mallplus.pms.service.IPmsGiftsService;
import com.zrp.mallplus.pms.service.IPmsProductConsultService;
import com.zrp.mallplus.pms.service.IPmsProductService;
import com.zrp.mallplus.sms.entity.*;
import com.zrp.mallplus.sms.mapper.SmsGroupMapper;
import com.zrp.mallplus.sms.mapper.SmsGroupMemberMapper;
import com.zrp.mallplus.sms.mapper.SmsGroupRecordMapper;
import com.zrp.mallplus.sms.service.*;
import com.zrp.mallplus.sms.vo.SmsCouponHistoryDetail;
import com.zrp.mallplus.ums.entity.*;
import com.zrp.mallplus.ums.mapper.SysAppletSetMapper;
import com.zrp.mallplus.ums.mapper.UmsIntegrationChangeHistoryMapper;
import com.zrp.mallplus.ums.mapper.UmsIntegrationConsumeSettingMapper;
...
import com.zrp.mallplus.util.ArithmeticUtil;
import com.zrp.mallplus.util.DateUtils;
import com.zrp.mallplus.util.applet.TemplateData;
import com.zrp.mallplus.util.applet.WX_TemplateMsgUtil;
...
//       emailUtil.sendEmailDealThank(EMAIL_SENDER,"支付待审核处理",tokenName,token,tbThanks);
        return count;
    }
     ...
     */
    @Override
    public int wxOrderPay(String paymentId){
        OmsOrder tbOrder = orderMapper.findBypaymentId(paymentId);
        if (tbOrder == null) {
            throw new ApiMallPlusException("订单不存在");
        }
        tbOrder.setId(tbOrder.getId());
        tbOrder.setStatus(OrderStatus.TO_DELIVER.getValue());
        tbOrder.setPaymentTime(new Date());
        tbOrder.setModifyTime(new Date());
        if (orderMapper.updateById(tbOrder) != 1) {
        ...
        //恢复所有下单商品的锁定库存，扣减真实库存
        OmsOrderItem queryO = new OmsOrderItem();
        queryO.setOrderId(tbOrder.getId());
        List<OmsOrderItem> list = orderItemService.list(new QueryWrapper<>(queryO));
        int count = orderMapper.updateSkuStock(list);
        return count;
    }
    ...
    @Override
    public OmsOrder getOrderInfo(TbThanks tbThanks){
        OmsOrder tbOrder = orderMapper.selectById(tbThanks.getOrderId());
        return tbOrder;
    }
        ...
        if (ValidatorUtils.notEmpty(smsGroupActivity.getGoodsIds())) {
            ...
            if (productList != null && productList.size() > 0) {
                ...
                for (PmsProduct product : productList) {
                    ...
                    omsCartItem.setPic(product.getPic());
    ...
    @Override
    public List<OmsOrder> findAll(Long memberId) {
        return orderMapper.findAll(memberId);
    }



    @Override
    public OmsOrder findByOrderSn(Long id) {
        return orderMapper.findByOrderSn(id);
    }
    ...
    public CommonResult generateOrder(OrderParam orderParam) {
        ...
        if (orderParam.getOrderType() == 3) {
            ...
            if ("1".equals(type)) {
                ...
                PmsProduct pmsProduct1 = productService.getById(goodsId);
//                Long cartId = Long.valueOf(orderParam.getCartId());
//                OmsCartItem omsCartItem = cartItemService.selectById(cartId);
                if (pmsProduct1 != null) {
                    OmsCartItem omsCartItem = new OmsCartItem();
                    omsCartItem.setProductId(pmsProduct1.getId());
                    omsCartItem.setMemberId(currentMember.getId());
                    omsCartItem.setQuantity(orderParam.getTotal());
                    BigDecimal price = pmsProduct1.getPrice();
                    Integer total = orderParam.getTotal();
                    String price1 = String.valueOf(price);
                    String valueOf = String.valueOf(total);
                    String multiplicationStr = ArithmeticUtil.multiplicationStr(price1, valueOf, 0);
                    String multiplicationStr1 = ArithmeticUtil.multiplicationStr(multiplicationStr, "100", 0);
                    BigDecimal bigDecimal = new BigDecimal(multiplicationStr1);
                    omsCartItem.setPrice(bigDecimal);
                    omsCartItem.setPic(pmsProduct.getPic());
...
//                      UmsMember member = memberService.getById(currentMember.getInvitecode());
    ...
    @Override
    public List<OmsOrder> findListBy13(Long menberId) {
        return  orderMapper.findListBy13(menberId);
    }

    @Override
    public List<OmsOrder> findListBy14(Long menberId) {
        return  orderMapper.findListBy14(menberId);
    }
    ...
    private void createCartObj(OrderParam orderParam, List<OmsCartItem> list, OmsCartItem cartItem, PmsProduct pmsProduct) {
        if (ValidatorUtils.notEmpty(orderParam.getSkuId())) {
            ...
            cartItem.setPic(pmsSkuStock.getPic());
            ...
            cartItem.setQuantity(orderParam.getTotal());
            cartItem.setPic(pmsProduct.getPic());
            cartItem.setCreateDate(new Date());
            cartItem.setMemberId(memberService.getNewCurrentMember().getId());
            cartItem.setProductCategoryId(pmsProduct.getProductCategoryId());
            cartItem.setProductBrand(pmsProduct.getBrandName());
...
//            orderItem.setGiftGrowth(cartPromotionItem.getGrowth());
    ...
    @Override
    public Object orderComment(Long orderId, Long omsOrderItemId,Long goodsId,Integer score,String[] images,String textarea) {
        ...
        try {
                ...
                pmsComment.setProductId(omsOrderItemId);
                pmsComment.setContent(textarea);
                pmsComment.setStar(score);
                pmsComment.setPics(Arrays.toString(images));
                pmsComment.setCreateTime(new Date());
                pmsComment.setOrderNo(orderId);
                if (ValidatorUtils.empty(textarea) && ValidatorUtils.empty(images)) {
                    return new CommonResult().failed("空");
                } else {
            ...
            omsOrderItem.setId(goodsId);
            omsOrderItem.setProductId(omsOrderItemId);
            omsOrderItem.setStatus(OrderStatus.TRADE_SUCCESS.getValue());
            iOmsOrderItemService.updateById(omsOrderItem);
            if (orderService.updateById(omsOrder) && iOmsOrderItemService.updateById(omsOrderItem)) {
                OmsOrderOperateHistory history = new OmsOrderOperateHistory();
                history.setStoreId(omsOrder.getStoreId());
                history.setOrderId(omsOrder.getId());
                history.setOrderItemId(goodsId);
                history.setCreateTime(new Date());
                history.setOperateMan("shop");
                history.setPreStatus(OrderStatus.TO_COMMENT.getValue());
                history.setOrderStatus(OrderStatus.TRADE_SUCCESS.getValue());
                history.setNote("商品评价");
                orderOperateHistoryService.save(history);
                memberService.addIntegration(member.getId(),20,1,"评论增加积分",AllEnum.ChangeSource.evaluate.code(),member.getUsername());  //评论增加积分
    ...
    public OmsOrder blancePay(PayParam payParam) {
        ...
        orderOperateHistoryService.save(history);
        memberService.addIntegration(userDO.getId(),
                order.getPayAmount().multiply(new BigDecimal("0.1")).intValue(),
                1,
                "余额支付添加积分",
                AllEnum.ChangeSource.order.code(),
                userDO.getUsername());
    ...
    @Transactional
    @Override
    public Object applyRe(Long itemId,String returnReson,String items,Integer type,String desc,String[] images,BigDecimal returnAmount) {
        try { if (ValidatorUtils.empty(items) ||items.split(",").length<1){
                return new CommonResult().failed("添加失败");
            }
            for (String id : items.split(",")){
                OmsOrder omsOrder = orderService.getById(id);
                omsOrder.setStatus(13);
                ...
                apply.setCompanyAddressId(omsOrder.getReceiverId());
                apply.setReturnAmount(returnAmount);
                apply.setDescription(desc);
                apply.setProductId(itemId);
                apply.setOrderSn(omsOrder.getOrderSn());
                Long valueOf = Long.valueOf(id);
                apply.setOrderId(valueOf);
                apply.setMemberUsername(member.getUsername());
                apply.setProductPrice(omsOrder.getTotalAmount());
                apply.setProductRealPrice(omsOrder.getPayAmount());
                apply.setProofPics(org.apache.commons.lang3.StringUtils.join(images, ","));
                apply.setReason(desc);
                apply.setType(type);
                ...
                OmsOrderOperateHistory history = new OmsOrderOperateHistory();
                history.setOrderId(valueOf);
                history.setCreateTime(new Date());
                history.setOperateMan("shop");
                history.setOrderItemId(itemId);
                //  //0换货 1退钱 2退货3 退钱退货
             if (apply.getType()==0){
                 history.setNote("换货");
