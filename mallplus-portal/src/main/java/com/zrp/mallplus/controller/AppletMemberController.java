...
import com.zrp.mallplus.pms.service.IPmsSmallNaviconCategoryService;

import com.zrp.mallplus.sms.entity.SmsCoupon;
import com.zrp.mallplus.sms.entity.SmsHomeAdvertise;
import com.zrp.mallplus.sms.mapper.SmsFlashPromotionSessionMapper;
import com.zrp.mallplus.sms.mapper.SmsHomeNewProductMapper;
import com.zrp.mallplus.sms.mapper.SmsHomeRecommendProductMapper;
...
public class AppletMemberController  {
    ...
    @IgnoreAuth
    @ApiOperation("登录注册")
    @SysLog(MODULE = "applet", REMARK = "小程序登录注册")
    @PostMapping("login_by_weixin1")
    public Object loginByWeixinNew(@RequestBody AppletLoginParam param) {
        try {
            Object byWeixin1 = memberService.loginByWeixin1(param);
            JSONObject jsonObject = JSONObject.fromObject(byWeixin1);
            Object data = jsonObject.get("data");
            JSONObject jsonObject1 = JSONObject.fromObject(data);
            String openid = jsonObject1.getString("openid");
            UmsMember umsMember = memberService.queryByOpenId(openid);
            String format = String.format(Rediskey.MEMBER, umsMember.getUsername());
            redisService.set(format, JsonUtils.objectToJson(umsMember));
            redisService.expire(format,24*60*60);
            return byWeixin1;
        } catch (Exception e) {
    ...
    @IgnoreAuth
    @SysLog(MODULE = "applet", REMARK = "小程序登录")
    ...
    public Object setUserInfo(@RequestBody AppletLoginParam userInfoParam) {
        ...
        user.setId(userId);
        user.setIcon(userInfoParam.getIcon());
        user.setGender(userInfoParam.getGender());
        user.setNickname(userInfoParam.getNickName());
        user.setBirthday(userInfoParam.getBrithDay());
        user.setCity(userInfoParam.getLocalCity());
        boolean update = memberService.updateById(user);
        if (!update){
            return new CommonResult().validateFailed("失败");
        }
        UmsMember username = memberService.getByUsername(umsMember.getUsername());
        String format = String.format(Rediskey.MEMBER, umsMember.getUsername());
        redisService.set(format, JsonUtils.objectToJson(username));
        redisService.expire(format,24*60*60);
    ...
    public Object getWxPhone(@RequestBody PhoneDto phoneDto) {
        try {
            String phone = memberService.getWxPhone(phoneDto.getOpenId(), phoneDto.getKeyStr() ,phoneDto.getIvStr() ,phoneDto.getEncDataStr());
            if (phone != null) {
...
*/
    ...
    public Object user() {
        UmsMember umsMember = memberService.getNewCurrentMember();
        Date birthday = umsMember.getBirthday();
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd");
        String format = simpleDateFormat.format(birthday);
        umsMember.setBirthDays(format);
